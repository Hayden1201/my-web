...
let isAutoColorActive = true;
let isRainbowModeActive = false;
let isFrozen = false; // New state for freezing the shape
let morphTarget = 0;
let morphProgress = 0;

const particleCount = 10000;

// --- START: New 10-Color Rainbow Palette ---
...
// --- END: New 10-Color Rainbow Palette ---
...
function init() {
  ...
  // --- START: Apply default theme settings on load ---
  setTheme('molten', true); // true indicates it's the initial setup
  // --- END: Apply default theme settings on load ---
...
function createUI() {
  ...
  // --- START: New "Auto Color" Button ---
  const autoColorBtn = document.createElement('button');
  autoColorBtn.className = 'theme-btn';
  autoColorBtn.dataset.theme = 'auto';
  autoColorBtn.textContent = 'Auto Color';
  autoColorBtn.addEventListener('click', () => {
    isAutoColorActive = true;
    isRainbowModeActive = false; // Deactivate rainbow mode
    currentTheme = 'molten'; // Reset to a base theme for bloom settings
    updateButtonStates();
    updateParticleColorsAndSizes();
  });
  themeSelector.appendChild(autoColorBtn);

  // --- START: New "Rainbow" Button ---
  const rainbowBtn = document.createElement('button');
  rainbowBtn.className = 'theme-btn';
  rainbowBtn.dataset.theme = 'rainbow';
  rainbowBtn.textContent = 'Rainbow';
  rainbowBtn.addEventListener('click', () => {
    isRainbowModeActive = true;
    isAutoColorActive = false; // Deactivate auto color mode
    currentTheme = 'molten'; // Reset to a base theme for bloom settings
    updateButtonStates();
    updateParticleColorsAndSizes();
  });
  themeSelector.appendChild(rainbowBtn);
  // --- END: New "Rainbow" Button ---
  ...
  // --- START: New "Freeze" Toggle ---
  const freezeToggleOption = document.createElement('div');
  freezeToggleOption.className = 'toggle-option';

  const freezeToggleLabel = document.createElement('label');
  freezeToggleLabel.className = 'toggle-switch';

  const freezeToggleInput = document.createElement('input');
  freezeToggleInput.type = 'checkbox';
  freezeToggleInput.id = 'freezeToggle';
  freezeToggleInput.checked = false; // Default to not frozen
  freezeToggleInput.addEventListener('change', (e) => {
    isFrozen = e.target.checked;
    // If unfreezing, ensure the main animation toggle is on
    if (!isFrozen) {
  ...
  const freezeToggleSlider = document.createElement('span');
  freezeToggleSlider.className = 'toggle-slider';

  freezeToggleLabel.appendChild(freezeToggleInput);
  freezeToggleLabel.appendChild(freezeToggleSlider);

  const labelForFreezeToggle = document.createElement('label');
  labelForFreezeToggle.htmlFor = 'freezeToggle';
  labelForFreezeToggle.textContent = 'Freeze';

  freezeToggleOption.appendChild(freezeToggleLabel);
  freezeToggleOption.appendChild(labelForFreezeToggle);
  controlsDiv.appendChild(freezeToggleOption);
  // --- END: New "Freeze" Toggle ---
}
...
function getAttributesForParticle(i) {
  ...
  if (isAutoColorActive) {
    // Auto Color Cycling Logic
    // Increased multipliers for a more significant and faster color change
    const hue = (t * 1.0 + time * 0.04) % 1.0;
    ...
    // --- START: New Rainbow Edge Color Logic ---
    const numEdges = 10;
    const edgeIndex = Math.floor(t * numEdges);
    const colorIndex = (edgeIndex + rainbowColorOffset) % numEdges;
    baseColor = rainbowPalette[colorIndex];
    // --- END: New Rainbow Edge Color Logic ---
  } else {
    // Original theme-based color logic
    ...
    baseColor = new THREE.Color().lerpColors(color1, color2, blendFactor);
  }
...
function animateParticles() {
  ...
  if (isFrozen) {
    for (let i = 0; i < particleCount; i++) {
      ...
      // Snap particles to their final position
      positions[i3] = homeX;
      positions[i3 + 1] = homeY;
      positions[i3 + 2] = homeZ;

      // Set a stable color and size
      const { color: baseParticleColor, size: baseParticleSize } = getAttributesForParticle(i);
      particleColors[i3] = baseParticleColor.r;
      particleColors[i3 + 1] = baseParticleColor.g;
      particleColors[i3 + 2] = baseParticleColor.b;
      particleSizes[i] = baseParticleSize;
    }
  ...
  // --- START: Logic to detect animation cycle completion ---
  const disintegrationCycleTime = 20.0;
  const totalCycles = Math.floor((time * 0.6) / disintegrationCycleTime);
  if (totalCycles > lastCycleCount) {
    lastCycleCount = totalCycles;
    rainbowColorOffset = (rainbowColorOffset + 1) % 10; // Shift the color offset
  }
  // --- END: Logic to detect animation cycle completion ---
...
function setTheme(themeName, isInitialSetup = false) {
  if (!themes[themeName]) return;
  
  // --- START: Preserve default mode on initial load ---
  if (!isInitialSetup) {
    isAutoColorActive = false; // Disable auto color when a specific theme is chosen
    isRainbowModeActive = false; // Disable rainbow mode
  }
  // --- END: Preserve default mode on initial load ---
...
// --- START: New function to manage button active states ---
function updateButtonStates() {
  document.querySelectorAll('.theme-btn').forEach((btn) => {
    if (isAutoColorActive) {
      btn.classList.toggle('active', btn.dataset.theme === 'auto');
    } else if (isRainbowModeActive) {
      btn.classList.toggle('active', btn.dataset.theme === 'rainbow');
    } else {
